...
public class WarehouseInventoryInController {
    ...
    @OpLog(module = "仓库管理", operation = "修改入库记录")
    public ResponseEntity<ApiResponse<Void>> update(@PathVariable Long id, @RequestBody InventoryInCreateRequest request) {
        inventoryInService.update(id, request);
        return ResponseEntity.ok(ApiResponse.success(null));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('SUPER_ADMIN')")
    @OpLog(module = "仓库管理", operation = "删除入库记录")
    public ResponseEntity<ApiResponse<Void>> delete(@PathVariable Long id) {
        inventoryInService.delete(id);
        return ResponseEntity.ok(ApiResponse.success(null));
    }
